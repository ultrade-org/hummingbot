...
class UltradeExchange(ExchangeBase):
    ...
    def __init__(self,
        ...
        self._user_stream_tracker = UserStreamTracker(
            data_source=UltradeAPIUserStreamDataSource(
                auth=self._auth,
                domain=self._domain,
                throttler=self._throttler,
                api_factory=self._api_factory,
    ...
    def status_dict(self) -> Dict[str, bool]:
        ...
        """
        return {
            "symbols_mapping_initialized": UltradeAPIOrderBookDataSource.trading_pair_symbol_map_ready(
                domain=self._domain),
    ...
    async def check_network(self) -> NetworkStatus:
        ...
        """
        try:
            print('check net ', CONSTANTS.DEFAULT_DOMAIN)
        ...
        while True:
            try:
                ...
                self.logger().network("Unexpected error while fetching trading rules.", exc_info=True,
                                      app_warning_msg="Could not fetch new trading rules from Ultrade. "
    ...
    async def _format_trading_rules(self, exchange_info_dict: Dict[str, Any]) -> List[TradingRule]:
        ...
        """
        print('exchange_info_dict ', exchange_info_dict)
        # TODO check this func
        trading_pair_rules = exchange_info_dict.get("data", [])
        print('trading_pair_rules', trading_pair_rules)
        retval = []
        for rule in trading_pair_rules:
            try:
                trading_pair = await UltradeAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
                    symbol=rule.get("pair_name"),
                    domain=self._domain,
                    api_factory=self._api_factory,
                    throttler=self._throttler,
                    time_synchronizer=self._time_synchronizer)
    ...
    async def _get_pairs(self):
        ...
        pairs_info = await self._api_request(
            method=RESTMethod.POST,
            path_url=CONSTANTS.PAIR_PATH_URL,
            domain=CONSTANTS.DEFAULT_DOMAIN,
            data=pair_req_body,
            is_auth_required=True
        )
    ...
    async def _update_time_synchronizer(self):
        try:
            ...
            self.logger().exception("Error requesting time from Ultrade server")
            raise
    ...
    async def get_last_traded_prices(self, trading_pairs: List[str]) -> Dict[str, float]:
        ...
        return await UltradeAPIOrderBookDataSource.get_last_traded_prices(
            trading_pairs=trading_pairs,
            domain=self._domain,
            api_factory=self._api_factory,
            throttler=self._throttler,
