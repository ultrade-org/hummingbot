...
class UltradeAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def exchange_symbol_associated_to_pair(
        ...
        """
        symbol_map = await UltradeAPIOrderBookDataSource.trading_pair_symbol_map(
            domain=domain,
            api_factory=api_factory,
            throttler=throttler,
            time_synchronizer=time_synchronizer)
        ...
        while True:
            try:
                ...
                for trades in json_msg["data"]:
                    trade_msg: OrderBookMessage = UltradeOrderBook.trade_message_from_exchange(
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        print('seconds_until_next_ping ', seconds_until_next_ping)
    ...
    async def _subscribe_channels(self, ws: WSAssistant):
        ...
        try:
            for trading_pair in self._trading_pairs:
                ...
                trade_payload = f'42["subscribe", {json.dumps(symbol_payload)}]'
                subscribe_trade_request: WSPlainTextRequest = WSPlainTextRequest(payload=trade_payload)
    ...
    async def _process_ob_snapshot(self, snapshot_queue: asyncio.Queue):
        print('REQUEST TO _process_ob_snapshot ')
        ...
        while True:
            try:
                ...
                trading_pair = await UltradeAPIOrderBookDataSource.trading_pair_associated_to_exchange_symbol(
                    ...
                    throttler=self._throttler,
                    time_synchronizer=self._time_synchronizer)
                order_book_message: OrderBookMessage = UltradeOrderBook.snapshot_message_from_exchange_websocket(
    ...
    async def _take_full_order_book_snapshot(self, trading_pairs: List[str], snapshot_queue: asyncio.Queue):
        print('REQUEST TO _take_full_order_book_snapshot ')
